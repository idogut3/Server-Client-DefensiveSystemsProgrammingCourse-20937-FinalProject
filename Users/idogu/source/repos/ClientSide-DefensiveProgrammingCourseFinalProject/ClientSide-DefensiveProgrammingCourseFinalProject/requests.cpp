...
Bytes SendFileRequest::pack_request() const {
	...
	// Create a request that combines the header and the first SEND_FILE_REQUEST_HEADER_EXTRAS_SIZE bytes of the payload
	...
	request.insert(request.end(), packed_payload.begin(), packed_payload.begin() + header_extras_size); // Add first SEND_FILE_REQUEST_HEADER_EXTRAS_SIZE bytes of payload

	return request;
}

void SendFileRequest::sendFileData(tcp::socket& sock) {
	Bytes total_file_data_bytes = stringToBytes(this->getPayload()->get_encrypted_file_content());
	size_t total_bytes_to_send = total_file_data_bytes.size();
	size_t total_packets_to_send = this->getPayload()->get_total_packets() - 1; // already sent 1 packet
	...
	while (bytes_sent < total_bytes_to_send) {
		...
		size_t bytes_to_send = (bytes_remaining < CONTENT_SIZE_PER_PACKET) ? bytes_remaining : CONTENT_SIZE_PER_PACKET;

		// Send the chunk
		boost::system::error_code error_code;
		size_t current_bytes_sent = sock.send(boost::asio::buffer(total_file_data_bytes.data() + bytes_sent, bytes_to_send), 0, error_code);

		if (error_code) {
			std::cerr << "Error sending data: " << error_code.message() << std::endl;
			return; // Exit on error
		}

		// Update the count of bytes sent
		bytes_sent += current_bytes_sent;
		total_packets_to_send--; // Decrease the number of packets left to send
	}

	std::cout << "Finished sending file data. Total bytes sent: " << bytes_sent << std::endl;
