...
SendFilePayload::SendFilePayload(uint32_t content_size, uint32_t orig_file_size, uint16_t total_packets, const char* file_name, string& encrypted_file_content)
	...
	errno_t result = strcpy_s(this->file_name, sizeof(this->file_name), file_name);
	if (result != 0) {
		std::cerr << "Error copying file name in SendFilePayload:: source is too long!" << std::endl;
		this->file_name[0] = '\0'; // Ensure file_name is empty on error
	}
}
